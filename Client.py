...
def handshake():
	...
	synAckPacket = server.recv(connInfo.get('packetSize')).decode('utf-8')
	synAckPacket = verifyChecksum(synAckPacket)
	if synAckPacket == None:
		return
	if synAckPacket[0:3] != SYNACK or int(synAckPacket[12:19]) != connInfo.get('synSeq')+1:
		return
	connInfo.update({'packetSize': int(synAckPacket[3:5])})
	ackSeq = int(synAckPacket[5:12])
	connInfo.update({'ackSeq': ackSeq})
	sendPacket(flags=ACK, ack=ackSeq+1)
...
def receiveMessage():
	...
	while True:
		packet = server.recv(connInfo.get('packetSize')).decode('utf-8')
		packet = verifyChecksum(packet)
		if packet == None:
			continue
		syn = int(packet[5:12])
